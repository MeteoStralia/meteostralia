...
def pipeline_sunshine(df):

    print('\n\ndébut du preprocessing des colonnes Sunshine...', end = '\n\n')

    pipeline_sun = Pipeline([
        ('windgustspeed_transformer', sunshine_transformer(col_select='Sunshine')),
    ])

    pipeline_sun.fit_transform(df)

    print(emoji.emojize('\n ...fin du preprocessing de la colonne  Sunshine :saluting_face:'), end = '\n\n')
